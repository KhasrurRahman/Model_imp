...
def grid_search_nn(X_train, X_test, y_train, y_test,
                   ...
                   activation_options=['leaky_relu', 'tanh'],
                   dropout_rates=[0.1, 0.2]):
    
    best_acc = 0
    best_config = None

    for num_layers, neurons, act_fn, dropout_rate in product(layer_options, neurons_options, activation_options, dropout_rates):
        print("\n" + "-"*60)
        print(f"Config: Layers={num_layers}, Neurons={neurons}, Activation={act_fn}, Dropout={dropout_rate}")
        
        nn = NeuralNetwork()
        input_dim = X_train.shape[1]
        
        # Input layer
        nn.add_layer(input_size=input_dim, output_size=neurons, activation=act_fn)
        nn.add_dropout_layer(rate=dropout_rate)

        # Hidden layers
        for _ in range(num_layers - 1):
            nn.add_layer(input_size=neurons, output_size=neurons, activation=act_fn)
            nn.add_dropout_layer(rate=dropout_rate)
        
        # Output layer
        nn.add_layer(input_size=neurons, output_size=2, activation=None)

        print(f"Total Parameters: {count_parameters(nn)}")
        print(f"Estimated RAM: {estimate_ram(nn):.2f} MB")

        nn.train(X_train, y_train, epochs=250, learning_rate=0.01, batch_size=32)
        ...
        if acc > best_acc:
            best_acc = acc
            best_config = (num_layers, neurons, act_fn, dropout_rate)
...
if __name__ == "__main__":
    for dataset_name, loader in [("credit", load_credit_data), ("voting", load_voting_data)]:
        print(f"\n{'='*30}\nLLM NN on {dataset_name.title()} Dataset\n{'='*30}")
